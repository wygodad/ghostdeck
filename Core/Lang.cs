...
public static class Lang
{
    ...
    private static void L12(Dictionary<string, string[]> m)
    {
        ...
        m["pt_intro"]         = new[] { "This sub-tab is the only one that measures rather than just reads. It runs the same load in SILENT, BALANCED and EXTREME, recording temperatures, fan speed, CPU clock and how much work the processor actually got done, once a second. That way a report carries numbers instead of impressions. MSI Center is not needed for any of it.", "Ta podzakładka jako jedyna mierzy, a nie tylko odczytuje. Uruchamia to samo obciążenie w SILENT, BALANCED i EXTREME i co sekundę zapisuje temperatury, obroty wentylatorów, zegar procesora oraz to, ile pracy procesor faktycznie wykonał. Dzięki temu w zgłoszeniu są liczby zamiast wrażeń. MSI Center nie jest do niczego potrzebny.", "Dieser Unterreiter ist der einzige, der misst und nicht nur ausliest. Er lässt dieselbe Last in SILENT, BALANCED und EXTREME laufen und schreibt jede Sekunde Temperaturen, Lüfterdrehzahl, CPU-Takt und die tatsächlich geleistete Arbeit des Prozessors mit. So stehen im Bericht Zahlen statt Eindrücken. MSI Center wird dafür an keiner Stelle gebraucht.", "Ce sous-onglet est le seul à mesurer, et pas seulement à lire. Il exécute la même charge en SILENT, BALANCED et EXTREME, en relevant chaque seconde les températures, la vitesse des ventilateurs, la fréquence du processeur et le travail que celui-ci a réellement accompli. Ainsi le rapport contient des chiffres et non des impressions. MSI Center n'est nécessaire à aucun moment.", "Esta subpestaña es la única que mide en lugar de solo leer. Ejecuta la misma carga en SILENT, BALANCED y EXTREME, y registra cada segundo las temperaturas, la velocidad del ventilador, la frecuencia del procesador y cuánto trabajo completó realmente. Así el informe lleva números en lugar de impresiones. No hace falta MSI Center para nada de esto.", "这个子选项卡是唯一进行实测而不只是读取的页面。它在 SILENT、BALANCED 和 EXTREME 下运行相同的负载，每秒记录一次温度、风扇转速、处理器频率以及处理器实际完成的工作量。这样报告里给出的是数字，而不是主观感受。整个过程都不需要 MSI Center。", "Esta subguia é a única que mede, em vez de apenas ler. Ela roda a mesma carga em SILENT, BALANCED e EXTREME e registra, uma vez por segundo, as temperaturas, a rotação das ventoinhas, a frequência do processador e quanto trabalho ele realmente concluiu. Assim o relatório traz números em vez de impressões. O MSI Center não é necessário para nada disso.", "Эта вкладка единственная, где идёт измерение, а не просто чтение. Она запускает одну и ту же нагрузку в SILENT, BALANCED и EXTREME и раз в секунду записывает температуру, обороты вентилятора, частоту процессора и то, сколько работы процессор реально выполнил. Поэтому в отчёте цифры, а не впечатления. MSI Center для всего этого не нужен." };
        ...
        m["pt_warn_heat"]    = new[] { "For a minute per profile every processor core runs flat out, so the laptop gets hot and the fans get loud. That is the measurement, not a fault. Allow about five minutes, and leave the machine alone while it runs: anything else you do competes with the test and spoils the numbers.", "Przez minutę na każdy profil wszystkie rdzenie procesora liczą na pełnych obrotach, więc laptop się nagrzeje, a wentylatory będą głośne. Tak wygląda pomiar, to nie usterka. Zarezerwuj około pięciu minut i nie korzystaj w tym czasie z komputera, bo cokolwiek innego robisz, konkuruje z testem i psuje wyniki.", "Pro Profil laufen alle Prozessorkerne eine Minute lang unter Volllast, der Laptop wird also heiß und die Lüfter werden laut. Das gehört zur Messung, es ist kein Fehler. Plane etwa fünf Minuten ein und lass das Gerät währenddessen in Ruhe: Alles andere, was du nebenbei machst, konkurriert mit dem Test und verfälscht die Zahlen.", "Pendant une minute par profil, tous les cœurs du processeur tournent à fond, la machine chauffe donc et les ventilateurs deviennent bruyants. C'est la mesure, pas un défaut. Comptez environ cinq minutes et ne vous servez pas de la machine pendant le test : tout ce que vous faites d'autre entre en concurrence avec lui et fausse les chiffres.", "Durante un minuto por perfil, todos los núcleos del procesador trabajan al máximo, así que el equipo se calienta y los ventiladores suenan fuerte. Eso es la medición, no un fallo. Reserva unos cinco minutos y no uses el equipo mientras tanto: cualquier otra cosa que hagas compite con la prueba y estropea los números.", "每个配置文件下，所有处理器核心都会满载运行一分钟，因此笔记本会变烫，风扇会变吵。这是测量过程，不是故障。请预留大约五分钟，并在测试运行期间不要使用笔记本：你做的任何其他事情都会与测试争抢资源，让测量结果失真。", "Durante um minuto em cada perfil, todos os núcleos do processador trabalham a plena carga, então o notebook esquenta e as ventoinhas ficam barulhentas. Isso é a medição, não um defeito. Reserve cerca de cinco minutos e não use o notebook enquanto o teste roda: qualquer outra coisa que você fizer concorre com ele e estraga os números.", "По минуте на каждый профиль все ядра процессора работают на полную, поэтому ноутбук нагреется, а вентиляторы станут громкими. Так и выглядит измерение, это не сбой. Отведите около пяти минут и не пользуйтесь компьютером, пока идёт замер: любые ваши действия конкурируют с тестом и портят результаты." };
