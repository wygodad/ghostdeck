...
public static class Lang
{
    ...
    private static void L12(Dictionary<string, string[]> m)
    {
        ...
        m["pt_warn_heat"]     = new[] { "For a minute per profile every processor core runs flat out, so the laptop gets hot and the fans get loud. That is the measurement, not a fault. Allow about five minutes.", "Przez minutę na każdy profil wszystkie rdzenie procesora liczą na pełnych obrotach, więc laptop się nagrzeje, a wentylatory będą głośne. Tak wygląda pomiar, to nie usterka. Zarezerwuj około pięciu minut.", "Pro Profil laufen alle Prozessorkerne eine Minute lang unter Volllast, der Laptop wird also heiß und die Lüfter werden laut. Das gehört zur Messung, es ist kein Fehler. Plane etwa fünf Minuten ein.", "Pendant une minute par profil, tous les cœurs du processeur tournent à fond, la machine chauffe donc et les ventilateurs deviennent bruyants. C'est la mesure, pas un défaut. Comptez environ cinq minutes.", "Durante un minuto por perfil, todos los núcleos del procesador trabajan al máximo, así que el equipo se calienta y los ventiladores suenan fuerte. Eso es la medición, no un fallo. Reserva unos cinco minutos.", "每个配置文件下，所有处理器核心都会满载运行一分钟，因此笔记本会变烫，风扇会变吵。这是测量过程，不是故障。请预留大约五分钟。", "Durante um minuto em cada perfil, todos os núcleos do processador trabalham a plena carga, então o notebook esquenta e as ventoinhas ficam barulhentas. Isso é a medição, não um defeito. Reserve cerca de cinco minutos.", "По минуте на каждый профиль все ядра процессора работают на полную, поэтому ноутбук нагреется, а вентиляторы станут громкими. Так и выглядит измерение, это не сбой. Отведите около пяти минут." };
