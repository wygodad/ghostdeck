...
No - and here is the honest reasoning, so it does not need re-explaining every time. The MUX switch (routing the internal display straight to the discrete GPU instead of through the integrated one) does **not** live in the Embedded Controller register space this project works in. Vendor tools flip it through a different mechanism plus a mandatory reboot, and none of the community register maps this app builds on (msi-ec, MControlCenter) document a confirmed, safe way to do it. Writing guesses into hardware is the one thing this project refuses to do. If a verified path ever shows up in those maps, it becomes possible; today it is out of scope. Set it in MSI Center if you need it - GhostDeck does not touch that mechanism, so the two do not conflict. (First asked in [#70](../../../issues/70).)
