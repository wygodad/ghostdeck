...
No - but the flag is understandable, and here is how to verify it yourself. GhostDeck ticks several boxes that antivirus heuristics dislike: it's an unsigned, self-contained single-file exe (it self-extracts the .NET runtime), asks for administrator rights, talks to the Embedded Controller, registers global hotkeys and can update itself. Occasionally a single engine (typically a small one, via a generic "W32.Malware.*" heuristic name) flags it on VirusTotal while all the major engines stay clean - a classic false positive pattern. To verify a download: compare its SHA-256 with the asset on the [Releases](../../../releases) page (`certutil -hashfile GhostDeck.exe SHA256` in a terminal) - every release is built from the public source by GitHub Actions, so the code that produced the exe is fully auditable, and you can always build it yourself (see the README). If a *majority* of engines ever flag a file claiming to be GhostDeck, don't run it and tell us - that would not be our build.
