...
### 6.3 Kompletne „przepisy" na scenariusze (po korekcie — patrz sekcja 8; `0x34` ujednolicone wg §20)
...
> `0x34` jest dynamiczny i nie to capuje Silenta (robi to `0xD4=0x1D`). Wartości to kanoniczna receptura (`00` tylko w Extreme); pierwotny pomiar 2.0.x złapał Silent na `00`. Patrz §20.1.
...
# SILENT:  (0x34 dynamiczny; kanon to tu 0x01, 0x00 tylko w Extreme — patrz §20.1. To 0xD4=1D capuje moc)
...
## 21. Nakładka do gier i dodatkowe metryki sprzętowe

### 21.1 Nakładka

Odczepiany HUD zawsze-na-wierzchu (kafelek w zakładce Scenariusze / skrót `Ctrl+Shift+O`) do grania:
zwarta **karta** albo poziomy **pasek** z podglądem na żywo temperatur, RPM, % wentylatorów,
aktywnego profilu, Cooler Boost, obciążenia CPU/GPU, RAM/VRAM, baterii, zegara CPU i limitu
ładowania. Pełna konfiguracja w Ustawienia → **Nakładka do gier**: co pokazywać, przezroczystość i
rozmiar (szybkie presety **oraz** suwak do dowolnej wartości), układ (karta/pasek), pozycja w rogu
lub swobodne przeciąganie, tło wł/wył + kolor, opcje (zawsze na wierzchu, blokada/klik-through,
akcent = kolor profilu). Cały układ jest DPI-aware (skalowany przez `DeviceDpi`), więc jest poprawny
przy 125 % / 150 % itd.

- **Blokada / klik-through** (`Ctrl+Shift+L`): ustawia `WS_EX_TRANSPARENT`, więc mysz przechodzi do
  gry, a panelu nie da się przeciągać. Opacity jest ograniczone do `0.99`, żeby WinForms trzymał
  `WS_EX_LAYERED` — bez tego przy 100 % klik-through byłby po cichu ignorowany.
- **Tło wyłączone** korzysta z `TransparencyKey` (klucz koloru wypełnienia) — widać tylko napisy/ikony.
- **Pozycja jest zapamiętywana** (`OverlayX/Y`).

### 21.2 Skąd bierze się każda metryka (i zasada „bez sterownika jądra")

Obietnica projektu to **bez sterownika jądra, bez osłabiania zabezpieczeń Windows**. To ogranicza
sposób czytania metryk — a w nakładce do *gier* liczy się podwójnie (anti-cheat).

| Metryka | Źródło | Uwagi |
|---------|--------|-------|
| Temp CPU/GPU, RPM, % went. | EC MSI przez WMI | rdzeń aplikacji |
| Obciążenie CPU | `GetSystemTimes` | bez sterownika |
| Użycie RAM | `GlobalMemoryStatusEx` | bez sterownika |
| Bateria % / ładowanie | `SystemInformation.PowerStatus` | bez sterownika |
| **Użycie GPU %** | licznik PDH `\GPU Engine(*engtype_3D)\Utilization Percentage` (suma) | to samo źródło co Menedżer zadań |
| **Użycie VRAM** | licznik PDH `\GPU Adapter Memory(*)\Dedicated Usage` (suma) | bez sterownika |
| **Zegar CPU (przybl.)** | PDH `\Processor Information(_Total)\% Processor Performance` × częstotliwość bazowa (rejestr `~MHz`) | estymata, nie odczyt MSR |

Wszystkie liczniki PDH dodawane są przez **`PdhAddEnglishCounter`** (`Perf.cs`), więc ścieżki działają
na polskim (i dowolnym) Windows — angielskie nazwy liczników są niezależne od locale. Wszystko jest
zabezpieczone: przy błędzie getter zwraca `-1`, a UI pokazuje `—`. Wartości próbkowane co ~700 ms.

### 21.3 Czego NIE użyliśmy i dlaczego

Rozważane dla „pełnego" zestawu (zegar rdzenia GPU, dokładny zegar CPU per-rdzeń, FPS, frametime):

- **SDK vendorów (NVAPI / AMD ADLX)** — user-mode, bez sterownika; dałyby dokładny zegar GPU, VRAM,
  obciążenie. **Odłożone**, nie odrzucone: dwie natywne ścieżki per-vendor dla marginalnego zysku nad
  wartościami PDH, które już pokazujemy. Do wrócenia, gdyby zależało na dokładnym zegarze GPU.
- **Dokładny zegar CPU per-rdzeń (MSR)** — wymaga sterownika jądra (WinRing0). **Odrzucone**: łamie
  zasadę braku sterownika, a WinRing0 bywa flagowany przez anti-cheat (Vanguard/EAC) → ryzyko bana.
  Estymata PDH `% Processor Performance` pokrywa „czy CPU boostuje / dławi się?" bez tego.
- **LibreHardwareMonitor** — jedna biblioteka z bogatymi sensorami, ale **ładuje sterownik jądra
  WinRing0** dla sensorów CPU/płyty (ten sam konflikt anti-cheat / brak-sterownika), licencja MPL-2.0
  (copyleft plikowy), cięższa i **nie daje FPS/frametime**. **Odrzucone** jako droga domyślna; PDH
  daje GPU/VRAM bez sterownika, a NVAPI/ADLX byłoby czystszą drogą do zegarów, gdyby były potrzebne.
- **FPS / frametime** — nie ma prostego API na FPS cudzego procesu. Realne drogi: **pamięć
  współdzielona RTSS** (wymaga RivaTuner/Afterburner u usera) albo **PresentMon/ETW** (systemowe
  łapanie Present, tylko admin, bez wstrzykiwania). Oba to osobny, większy nakład; **nie wdrożone**.
  Własne hookowanie Present odrzucone (kruche, ryzyko anti-cheat).

**Wdrożone teraz:** użycie GPU %, VRAM, zegar CPU (przybl.), bateria — wszystko bez sterownika (PDH +
API Windows). **Planowane/opcjonalne:** FPS+frametime (RTSS lub PresentMon), dokładny zegar GPU
(NVAPI/ADLX).

### 21.4 Render per-piksel (niezależna alfa tła i treści)

Nakładka renderuje się **per-piksel** przez `UpdateLayeredWindow` na bitmapie 32-bpp ARGB z
premnożoną alfą (`OverlayForm.RenderLayered`), a nie przez zwykły `OnPaint`. `WS_EX_LAYERED` jest
stały (ustawiany w `CreateParams`); `Form.Opacity`/`TransparencyKey` **nie** są używane. Daje to w
jednym podejściu:

- **Niezależną alfę tła i treści** — dwa osobne suwaki (`OverlayOpacity` = treść, `OverlayBgOpacity` =
  tło), każdy z presetami. Treść rysowana jest nieprzezroczyście na własnej warstwie i komponowana z
  alfą treści; zaokrąglone tło wypełniane jest alfą tła. Można mieć ledwo widoczne tło i w pełni
  czytelny tekst.
- **Gładkie krawędzie** na dowolnym tle gry (AA w skali szarości na warstwie treści daje poprawną alfę
  per-piksel — koniec obwódek chroma key).
- **Miękki cień** pod treścią (warstwa treści przerysowana jako czarna sylwetka na ~½ alfy, przesunięta
  ~1 px), więc tekst jest czytelny nawet przy wyłączonym tle, na jasnych i ciemnych scenach.
- **Idealne zaokrąglone rogi** z kształtu alfy (bez przycinania `Region`).
- **Naturalny klik-through** — w pełni przezroczyste piksele same przepuszczają mysz do gry; blokada
  (`WS_EX_TRANSPARENT`) dodatkowo czyni całe okno przezroczystym dla myszy.

Kolejność komponowania klatki: tło (zaokrąglone, `bgAlpha`) → cień → treść (`contentAlpha`) → ramka +
uchwyt. Układ mierzony jest na `Graphics` w DPI ekranu, a bitmapy dostają `SetResolution(dpi, dpi)`,
więc czcionki w punktach renderują się identycznie jak zmierzony rozmiar przy dowolnym skalowaniu.

**Tryb przesuwania vs wyświetlania:** gdy odblokowana (przeciągalna) nakładka wymusza widoczną,
