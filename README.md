...
- 📊 **Power test** (Report / verify ⚑ → Power test) - measures your profiles instead of asking you to judge them by ear. The same all-core load runs in Silent, Balanced and Extreme while temperatures, fan speed, fan RPM, clock and the work the processor actually completes are sampled every second, so a model report carries a comparison table rather than impressions. On boards whose entry records a **fourth performance value** (the switch some MSI Center versions put inside the top scenario) it also probes whether that value can be set from outside, reading the register back after the write and again after the revert. Every address it may touch is listed before you start, it needs mains power and your consent, and your profile is restored at the end. Unlike the other two report wizards it does **not** need MSI Center
...
| Measured comparison of the power profiles under load | ❌ | ✅ *(Power test: same load in each profile, sampled every second)* |
